...
describe('server responses', () => {
  ...
  it('should respond to a POST request to save a background image', (done) => {
    fs.readFile(postTestFile, (err, fileData) => {
      ...
      let {req, res} = server.mock(path.join('background.jpg'), 'POST', fileData);
  ...
  it('should send back the previously saved image', (done) => {
    fs.readFile(postTestFile, (err, fileData) => {
      httpHandler.backgroundImageFile = path.join('.', 'spec', 'temp.jpg');
      let post = server.mock(path.join('background.jpg'), 'POST', fileData);

      httpHandler.router(post.req, post.res, () => {
        let get = server.mock(path.join('background.jpg'), 'GET');
